...
from app.core.contracts import ArchitecturePlan, ExecutionResult, ExecutionStatus, HumanDecisionType, RiskLevel, TaskSpec, WorkerSpec
...
class FakeWorkerAgent:
    def build_execution_plan(self, *, worker, task, context=None):
        return WorkerExecutionPlan(
            ...
            execution_task=WorkerExecutionTask(task=task, language="python", code="print('ok')"),
        )
...
def make_risky_orchestrator():
    ...
    coordinator = RuntimeCoordinator(session_manager=sessions, intake_service=intake, approval_engine=approvals)
    ...
    gate_a = approvals.request_gate(run_id=run.run_id, kind="ARCHITECTURE", title="Approve architecture", prompt="approve")
    ...
    decision_a = approvals.resolve_gate(gate_id=gate_a.gate_id, decision=HumanDecisionType.APPROVE, feedback="approved")
    coordinator.apply_architecture_decision(decision_a)

    registry = ToolRegistry()
    registry.register_capability(CapabilitySpec("protected", "Protected", "Requires explicit review"))
...
class M19ToolOrchestrationTests(unittest.TestCase):
    ...
    def test_risky_tool_pauses_before_execution(self):
        ...
        self.assertEqual(sessions.get_context(run_id).state, WorkflowState.WORKER_WAITING_HUMAN)
    ...
    def test_unknown_tool_fails_closed(self):
        orchestrator, sessions, run_id, _ = make_risky_orchestrator()
        sessions.set_architecture_plan(run_id, ArchitecturePlan(
            plan_id="missing-tool-plan",
            objective="Build a protected project",
            acceptance_criteria=("produce a successful execution",),
            workers=(WorkerSpec(
                worker_id="worker-risky",
    ...
    def test_gate_c_request_is_idempotent(self):
        orchestrator, _, run_id, approvals = make_risky_orchestrator()
        first = orchestrator.execute_run(run_id)
        gate_id = first.pending_gate_id
        with self.assertRaises(IntegratedOrchestrationError):
            orchestrator.execute_run(run_id)
        self.assertEqual(len(approvals.list_gates(run_id=run_id, kind="TOOL_RISK")), 1)
