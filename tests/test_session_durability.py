...
class SessionRepositoryDurabilityTests(unittest.TestCase):
    def exercise_repository(self, repository):
        ...
        manager.set_final_result(
            first.run_id,
            FinalResult(run_id=first.run_id, status="ready", summary="done"),
        )
    ...
    def test_sqlite_repository_survives_manager_restart(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn(run.run_id, {record.context.run_id for record in restarted.list_sessions()})
