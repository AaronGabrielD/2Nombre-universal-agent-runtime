...
class M27M28Tests(unittest.TestCase):
    ...
    def test_json_session_repository_round_trip(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            record.messages.append(
                __import__("app.session.models", fromlist=["SessionMessage"]).SessionMessage(
                    run_id=context.run_id, role="user", content="hello"
                )
            )
            record.architecture_plan = ArchitecturePlan(
                plan_id="p1", objective="test", workers=(
                    WorkerSpec("w1", "builder", "build", required_tools=("calculator",)),
                ),
            )
    ...
    def test_json_repository_uses_schema_version_and_atomic_file(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            path = os.path.join(temp_dir, context.run_id + ".json")
    ...
    def test_migration_runner_applies_each_version_once(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            database = os.path.join(temp_dir, "test.db")
            calls = []
