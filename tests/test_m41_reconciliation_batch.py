...
from app.execution.reconciliation_batch import RunExecutionReconciliationService
...
class M41ReconciliationBatchTests(unittest.TestCase):
    ...
    def test_backend_no_result_keeps_task_pending_and_never_allows_retry(self):
        ...
        self.assertTrue(all(item.status == ReconciliationStatus.BACKEND_UNAVAILABLE for item in result.items))
