...
class SupervisorService:
    ...
    M09 deliberately performs deterministic checks first. A future LLM-backed
    evaluator may add qualitative scoring, but it must not bypass these gates.
    """
    ...
    def evaluate(self, data: SupervisorInput) -> QAResult:
        ...
        execution_failures = [
            ...
            if result.status in {ExecutionStatus.ERROR, ExecutionStatus.TIMEOUT, ExecutionStatus.DENIED, ExecutionStatus.UNAVAILABLE}
        ]
        ...
        missing_criteria = not data.acceptance_criteria
        if missing_criteria:
