...
class Settings:
    ...
    def validate(self):
        ...
        if self.execution_gateway_url:
            ...
            if parsed.scheme not in {"http","https"} or not parsed.netloc: raise RuntimeConfigurationError("EXECUTION_GATEWAY_URL must be an absolute HTTP(S) URL")
        ...
        if self.min_workers<1: raise RuntimeConfigurationError("MIN_WORKERS must be >= 1")
        if self.max_workers<self.min_workers: raise RuntimeConfigurationError("MAX_WORKERS must be >= MIN_WORKERS")
        if self.max_workers>32: raise RuntimeConfigurationError("MAX_WORKERS exceeds the safety ceiling of 32")
        if self.max_uploads_per_message<1: raise RuntimeConfigurationError("MAX_UPLOADS_PER_MESSAGE must be >= 1")
        if self.max_upload_size_mb<1: raise RuntimeConfigurationError("MAX_UPLOAD_SIZE_MB must be >= 1")
        if self.default_execution_timeout_seconds<1: raise RuntimeConfigurationError("DEFAULT_EXECUTION_TIMEOUT_SECONDS must be >= 1")
...
def get_settings(*,reload=False):
    ...
    settings=Settings(gemini_api_key=os.getenv("GEMINI_API_KEY") or None,
        ...
        gemini_temperature=_float_env("GEMINI_TEMPERATURE",0.2), max_workers=_int_env("MAX_WORKERS",4,minimum=1),
        min_workers=_int_env("MIN_WORKERS",3,minimum=1), default_execution_timeout_seconds=_int_env("DEFAULT_EXECUTION_TIMEOUT_SECONDS",60,minimum=1),
        max_uploads_per_message=_int_env("MAX_UPLOADS_PER_MESSAGE",20,minimum=1), max_upload_size_mb=_int_env("MAX_UPLOAD_SIZE_MB",100,minimum=1),
        execution_backend=os.getenv("EXECUTION_BACKEND","colab").strip().lower(), execution_gateway_url=os.getenv("EXECUTION_GATEWAY_URL") or None,
