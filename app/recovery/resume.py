...
from app.execution.reconciliation import ExecutionReconciliation, ExecutionReconciliationService, ReconciliationStatus
...
class RecoveryResumeService:
    ...
    def resume(self, *, run_id: str, action: RecoveryAction, idempotency_key: str | None = None) -> RecoveryResumeResult:
        ...
        if checkpoint.action != action:
            raise RecoveryResumeError(f"recovery action mismatch: checkpoint requires {checkpoint.action.value}, got {action.value}")
        ...
        if action == RecoveryAction.REBUILD_ARCHITECTURE:
            if state == WorkflowState.REVISION:
                ...
                raise RecoveryResumeError(f"architecture rebuild requires ARCHITECTING or REVISION, got {state.value}")
            return RecoveryResumeResult(run_id, checkpoint, state)
        if action in {RecoveryAction.AWAIT_ARCHITECT_APPROVAL, RecoveryAction.AWAIT_HUMAN_GATE, RecoveryAction.AWAIT_FINAL_APPROVAL}:
            return RecoveryResumeResult(run_id, checkpoint, state)
        ...
        if action == RecoveryAction.RECONCILE_EXECUTION:
            ...
            if result.status == ReconciliationStatus.PENDING_BACKEND_CHECK:
                result = self.reconciliation.reconcile_backend(run_id=run_id, idempotency_key=idempotency_key)
            ...
            if result.status == ReconciliationStatus.FAILED:
                ...
                return RecoveryResumeResult(run_id, checkpoint, state, result)
            raise RecoveryResumeError(f"execution reconciliation did not establish terminal evidence: {result.status.value}")
