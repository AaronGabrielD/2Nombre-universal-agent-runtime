...
class WorkerRuntimeAdapter:
    def __init__(self,*,gateway:ExecutionGateway,session_manager:SessionManager,lease_service:ExecutionLeaseService|None=None,settings:Settings|None=None):
        self.gateway=gateway;self.sessions=session_manager;self.leases=lease_service or ExecutionLeaseService(session_manager=session_manager);self.settings=settings or getattr(gateway,"_settings",None) or get_settings()
    def execute_batch(self,*,batch:DispatchBatch,tasks:tuple[WorkerExecutionTask,...],authorization:ExecutionAuthorization,backend_id:str|None=None,parallel:bool=False)->tuple[ExecutionResult,...]:
        ...
        with ThreadPoolExecutor(max_workers=min(len(validated_tasks),self.settings.max_workers),thread_name_prefix="uar-worker") as pool:return tuple(f.result() for f in tuple(pool.submit(self._execute_one,batch=batch,item=item,authorization=authorization,backend_id=backend_id) for item in validated_tasks))
    def _validate_batch(self,*,batch,tasks):
        ...
        if any(w.run_id!=batch.run_id for w in batch.workers):raise WorkerRuntimeError("all batch workers must belong to batch.run_id")
        worker_ids={w.worker_id for w in batch.workers};task_workers={i.task.worker_id for i in tasks}
    ...
    def _execute_one(self,*,batch,item,authorization,backend_id):
        execution=ExecutionRequest(execution_id=f"exec-{uuid4().hex}",run_id=batch.run_id,worker_id=item.task.worker_id,language=item.language,code=item.code,timeout_seconds=item.timeout_seconds,needs_network=item.needs_network,environment=dict(item.environment));key=self.leases.idempotency_key(run_id=batch.run_id,worker_id=item.task.worker_id,task_id=item.task.task_id,language=item.language,code=item.code)
