...
class ColabExecutionBackend(ExecutionBackend):
    ...
    def __init__(self, *, base_url: str, token: str | None = None, timeout_seconds: int = 60,
                 execute_path: str = "/execute", available: bool = True) -> None:
    ...
    def info(self) -> ExecutionBackendInfo:
        return ExecutionBackendInfo(backend_id="colab", name="Google Colab HTTP execution backend", available=self._available)

    def execute(self, request: ExecutionRequest) -> ExecutionResult:
        payload = {"execution_id": request.execution_id, "run_id": request.run_id, "worker_id": request.worker_id,
                   "language": request.language, "code": request.code, "timeout_seconds": request.timeout_seconds,
                   "needs_network": request.needs_network, "environment": dict(request.environment)}
        body = json.dumps(payload).encode("utf-8")
        headers = {"Content-Type": "application/json", "Accept": "application/json"}
        if self._token:
            headers["Authorization"] = f"Bearer {self._token}"
        http_request = Request(f"{self._base_url}{self._execute_path}", data=body, headers=headers, method="POST")
        started = monotonic()
        try:
            ...
            detail = _read_error_body(exc, self.MAX_ERROR_BYTES)
            raise ColabBackendError(f"Colab service returned HTTP {exc.code}: {detail or exc.reason}") from exc
...
def _result_from_payload(payload: object, request: ExecutionRequest, elapsed_ms: int) -> ExecutionResult:
    ...
    try:
        ...
        raise ColabBackendError(f"invalid execution status from Colab: {payload.get('status')!r}") from exc
    ...
    if not isinstance(remote_duration, int) or isinstance(remote_duration, bool) or remote_duration < 0:
    ...
    return ExecutionResult(execution_id=request.execution_id, run_id=request.run_id, status=status,
        exit_code=exit_code, stdout=stdout, stderr=stderr, duration_ms=remote_duration,
        artifacts=artifacts, backend="colab")
...
def _parse_artifacts(raw_artifacts: object) -> tuple[ArtifactRef, ...]:
    ...
    if len(raw_artifacts) > 100:
        raise ColabBackendError("Colab response contains too many artifacts")
    parsed = []
    for item in raw_artifacts:
        ...
        mime_type, uri = item.get("mime_type"), item.get("uri")
        if not isinstance(artifact_id, str) or not artifact_id.strip() or not isinstance(name, str) or not name.strip():
        ...
        parsed.append(ArtifactRef(artifact_id=artifact_id, name=name, mime_type=mime_type, uri=uri))
    return tuple(parsed)
