...
class SessionManager:
    """Own run-scoped mutable state and serialize operations per process."""
    ...
    def create_session(self, *, metadata: dict[str, str] | None = None) -> RunContext:
        if metadata is not None and (
            ...
            raise ValueError("session metadata must be a string-to-string mapping")
    ...
    def transition(self, run_id: str, target: WorkflowState) -> RunContext:
        if not isinstance(target, WorkflowState):
            raise ValueError("target must be a WorkflowState")
        with self._lock:
    ...
    def add_decision(self, run_id: str, decision: HumanDecision) -> None:
        ...
        with self._lock:
            record = self.repository.get(run_id)
            existing = next(
                (item for item in record.decisions if item.gate_id == decision.gate_id),
                None,
            )
    ...
    def add_execution_result(self, run_id: str, result: ExecutionResult) -> None:
        ...
        with self._lock:
            record = self.repository.get(run_id)
            existing = next(
                (item for item in record.execution_results if item.execution_id == result.execution_id),
                None,
            )
            if existing is not None:
                if existing == result:
    ...
    def resolve_approval_gate(
        ...
        if (
            ...
            or decision.gate_id != gate.gate_id
        ...
        with self._lock:
            record = self.repository.get(run_id)
            index = next(
                (i for i, existing in enumerate(record.approval_gates) if existing.gate_id == gate.gate_id),
                None,
            )
            if index is None:
            ...
            if any(existing.gate_id == decision.gate_id for existing in record.decisions):
    ...
    def set_worker_output(self, run_id: str, output: WorkerOutput) -> None:
        if not isinstance(output.worker_id, str) or not output.worker_id.strip():
            raise ValueError("WorkerOutput.worker_id cannot be empty")
        ...
        if not isinstance(output.status, str) or not output.status.strip():
            raise ValueError("WorkerOutput.status cannot be empty")
        if not isinstance(output.output, dict):
            raise ValueError("WorkerOutput.output must be a mapping")
        with self._lock:
            record = self.repository.get(run_id)
