...
class ToolRegistry:
    ...
    def register_capability(self, capability: CapabilitySpec) -> None:
        ...
        with self._lock:
            if capability.capability_id in self._capabilities:
                raise ToolRegistryError(f"capability already registered: {capability.capability_id}")
    ...
    def unregister_capability(self, capability_id: str) -> None:
        with self._lock:
            ...
            in_use = sorted(tool.tool_id for tool in self._tools.values() if capability_id in tool.capabilities)
            if in_use:
                raise ToolRegistryError(f"capability is still required by registered tools: {in_use}")
    ...
    def register_tool(self, tool: ToolRegistration) -> None:
        ...
        with self._lock:
            ...
            if missing:
                raise ToolRegistryError(f"tool {tool.tool_id} references unregistered capabilities: {missing}")
    ...
    def list_tools(self, *, available_only: bool = False) -> tuple[ToolRegistration, ...]:
        with self._lock:
            tools = (tool for tool in self._tools.values() if not available_only or tool.available)
            return tuple(self._snapshot_tool(tool) for tool in sorted(tools, key=lambda item: item.tool_id))

    def tools_for_capabilities(self, capability_ids: Iterable[str], *, available_only: bool = True) -> tuple[ToolRegistration, ...]:
        ...
        with self._lock:
            ...
            for tool in self._tools.values():
                if available_only and not tool.available:
                    continue
                if requested_set.intersection(tool.capabilities):
                    matches.append(self._snapshot_tool(tool))
            return tuple(sorted(matches, key=lambda item: item.tool_id))

    def validate_requirements(self, *, required_tools: Iterable[str] = (), required_capabilities: Iterable[str] = (), available_only: bool = True) -> RegistryValidation:
        requested_tools = tuple(dict.fromkeys(required_tools))
        requested_capabilities = tuple(dict.fromkeys(required_capabilities))
        with self._lock:
            missing_capabilities = set(requested_capabilities) - set(self._capabilities)
            ...
            covered = set().union(*(set(tool.capabilities) for tool in resolved)) if resolved else set()
            # If capabilities were requested without explicit tools, resolve all available
            # concrete providers that cover the requested set. This is deterministic and
            # supports capabilities distributed across multiple tools.
            if requested_capabilities and not missing_capabilities:
                candidates = [tool for tool in self._tools.values() if (not available_only or tool.available)]
                while not set(requested_capabilities).issubset(covered):
                    remaining = set(requested_capabilities) - covered
                    best = max(
                        (tool for tool in candidates if tool.tool_id not in {item.tool_id for item in resolved}),
                        key=lambda tool: (len(remaining.intersection(tool.capabilities)), tool.tool_id),
                        default=None,
                    )
                    if best is None or not remaining.intersection(best.capabilities):
                        break
                    resolved.append(self._snapshot_tool(best))
                    covered.update(best.capabilities)

            missing_capabilities.update(set(requested_capabilities) - covered)
            return RegistryValidation(
                requested_tools=requested_tools,
                requested_capabilities=requested_capabilities,
                resolved_tools=tuple(sorted(resolved, key=lambda item: item.tool_id)),
                missing_tools=tuple(sorted(set(missing_tools_list))),
