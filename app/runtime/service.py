...
class ArchitectureCheckpoint:
    run_id:str; plan:ArchitecturePlan; gate_id:str
...
class RuntimeCoordinator:
    def __init__(self,*,session_manager=None,intake_service=None,architect=None,approval_engine=None,revision_service=None,recovery_resume_service=None,supervisor=None):
        self.sessions=session_manager or SessionManager(); self.intake=intake_service or IntakeService(session_manager=self.sessions); self.architect=architect
        self.approvals=approval_engine or HumanApprovalEngine(); self.revisions=revision_service or RevisionService(session_manager=self.sessions)
        self.recovery_resume=recovery_resume_service or RecoveryResumeService(session_manager=self.sessions); self.supervisor=supervisor or SupervisorService()
    def start_run(self,objective:str,*,files:tuple[IntakeFile,...]=(),metadata:dict[str,str]|None=None,inline_text_by_name:dict[str,str]|None=None)->IntakeResult:
        return self.intake.start_session(objective,files=files,metadata=metadata,inline_text_by_name=inline_text_by_name)
    def build_architecture(self,run_id:str,*,context:dict[str,Any]|None=None)->ArchitectureCheckpoint:
        ...
        inputs=tuple({"artifact_id":a.artifact_id,"name":a.name,"mime_type":a.mime_type,"uri":a.uri} for a in snapshot.artifacts)
        plan=self.architect.build_plan(ArchitectureInput(objective=objective,inputs=inputs,context=dict(context or {}))); self.sessions.set_architecture_plan(run_id,plan); self.sessions.transition(run_id,WorkflowState.WAITING_ARCHITECT_APPROVAL)
        gate=self.approvals.request_gate(run_id=run_id,kind="ARCHITECTURE",title="Approve architecture plan",prompt="Review the proposed architecture. Approve to authorize worker execution, modify/clarify to re-plan, or reject to stop the run.",context={"plan":to_dict(plan)})
    ...
    def record_supervisor_result(self,result:QAResult)->str:
        ...
        gate=self.approvals.request_gate(run_id=result.run_id,kind="FINAL",title="Approve final result",prompt="Supervisor passed QA. Human approval is required before this run becomes COMPLETED.",context={"qa_result":to_dict(result)},allowed_decisions=(HumanDecisionType.APPROVE,HumanDecisionType.MODIFY,HumanDecisionType.REJECT))
        return gate.gate_id
    def apply_final_decision(self,decision:HumanDecision,*,final_result:FinalResult|None=None)->WorkflowState:
        ...
        if decision.decision==HumanDecisionType.APPROVE:
            result=final_result or FinalResult(run_id=decision.run_id,status="approved",summary=decision.feedback or "Final result approved by human."); result.validate(); self.sessions.set_final_result(decision.run_id,result); self.sessions.transition(decision.run_id,WorkflowState.COMPLETED)
        ...
        return self.sessions.get_context(decision.run_id).state
    def record_revision(self,run_id:str,*,reason:str,source:str="runtime",feedback:str=""): return self.revisions.request_revision(run_id,reason=reason,source=source,feedback=feedback)
    def list_revisions(self,run_id:str): return self.revisions.list_revisions(run_id)
    ...
    def resume_recovery(self,*,run_id:str,action:RecoveryAction,idempotency_key:str|None=None)->RecoveryResumeResult:
        ...
        return result
    def _supervise_recovered_run(self,run_id:str)->QAResult:
        ...
        qa=self.supervisor.evaluate(SupervisorInput(run_id=run_id,objective=plan.objective,acceptance_criteria=plan.acceptance_criteria,worker_outputs=tuple(snapshot.worker_outputs.values()),execution_results=snapshot.execution_results,artifacts=tuple(to_dict(a) for a in snapshot.artifacts)))
        self.sessions.add_message(run_id,role="supervisor",content=qa.summary,metadata={"phase":"qa","source":"recovery","qa_result":to_dict(qa)})
        return qa
