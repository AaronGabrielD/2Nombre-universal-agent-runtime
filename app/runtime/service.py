...
class RuntimeCoordinator:
    def __init__(self, *, session_manager: SessionManager | None = None,
                 intake_service: IntakeService | None = None, architect: UniversalArchitect | None = None,
                 approval_engine: HumanApprovalEngine | None = None, revision_service: RevisionService | None = None,
                 recovery_resume_service: RecoveryResumeService | None = None) -> None:
        ...
        self.recovery_resume = recovery_resume_service or RecoveryResumeService(session_manager=self.sessions)

    def start_run(self, objective: str, *, files: tuple[IntakeFile, ...] = (), metadata: dict[str, str] | None = None,
                  inline_text_by_name: dict[str, str] | None = None) -> IntakeResult:
    ...
    def build_architecture(self, run_id: str, *, context: dict[str, Any] | None = None) -> ArchitectureCheckpoint:
        ...
        objective = next((m.content for m in reversed(snapshot.messages)
                          if m.role == "user" and m.metadata.get("phase") == "intake"), "")
        if not objective:
            raise RuntimeCoordinatorError("Run has no intake objective")
        inputs = tuple({"artifact_id": a.artifact_id, "name": a.name, "mime_type": a.mime_type, "uri": a.uri}
                       for a in snapshot.artifacts)
        plan = self.architect.build_plan(ArchitectureInput(objective=objective, inputs=inputs, context=dict(context or {})))
        ...
        gate = self.approvals.request_gate(
            run_id=run_id, kind="ARCHITECTURE", title="Approve architecture plan",
    ...
    def apply_architecture_decision(self, decision: HumanDecision) -> WorkflowState:
        ...
        if context.state != WorkflowState.WAITING_ARCHITECT_APPROVAL:
            raise RuntimeCoordinatorError(f"Architecture decision requires WAITING_ARCHITECT_APPROVAL, got {context.state.value}")
        ...
        if decision.decision == HumanDecisionType.APPROVE:
            ...
            self.revisions.request_revision(decision.run_id, reason=f"Architecture {decision.decision.value}", source="human", feedback=decision.feedback)
        else:
    ...
    def record_supervisor_result(self, result: QAResult) -> str:
        ...
        if context.state != WorkflowState.SUPERVISING:
            raise RuntimeCoordinatorError(f"Final approval requires SUPERVISING, got {context.state.value}")
        self.sessions.transition(result.run_id, WorkflowState.WAITING_FINAL_APPROVAL)
        gate = self.approvals.request_gate(run_id=result.run_id, kind="FINAL", title="Approve final result",
            prompt="Supervisor passed QA. Human approval is required before this run becomes COMPLETED.",
            context={"qa_result": to_dict(result)}, allowed_decisions=(HumanDecisionType.APPROVE, HumanDecisionType.MODIFY, HumanDecisionType.REJECT))
        return gate.gate_id
    ...
    def apply_final_decision(self, decision: HumanDecision, *, final_result: FinalResult | None = None) -> WorkflowState:
        ...
        if context.state != WorkflowState.WAITING_FINAL_APPROVAL:
            raise RuntimeCoordinatorError(f"Final decision requires WAITING_FINAL_APPROVAL, got {context.state.value}")
        if decision.decision not in {HumanDecisionType.APPROVE, HumanDecisionType.MODIFY, HumanDecisionType.REJECT}:
            raise RuntimeCoordinatorError("Invalid decision for final approval gate")
        self._validate_recorded_decision(decision)
        self.sessions.add_decision(decision.run_id, decision)
        if decision.decision == HumanDecisionType.APPROVE:
            if final_result is None:
                final_result = FinalResult(run_id=decision.run_id, status="approved", summary=decision.feedback or "Final result approved by human.")
            ...
            self.revisions.request_revision(decision.run_id, reason="Final result modification requested", source="human", feedback=decision.feedback)
        else:
            self.sessions.transition(decision.run_id, WorkflowState.REJECTED)
        return self.sessions.get_context(decision.run_id).state

    def record_revision(self, run_id: str, *, reason: str, source: str = "runtime", feedback: str = ""):
        try:
            return self.revisions.request_revision(run_id, reason=reason, source=source, feedback=feedback)
    ...
    def resume_recovery(self, *, run_id: str, action: RecoveryAction, idempotency_key: str | None = None) -> RecoveryResumeResult:
        try:
            return self.recovery_resume.resume(run_id=run_id, action=action, idempotency_key=idempotency_key)
        except RecoveryResumeError as exc:
            raise RuntimeCoordinatorError(str(exc)) from exc
