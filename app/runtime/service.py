...
class RuntimeCoordinator:
    ...
    def build_architecture(self,run_id:str,*,context:dict[str,Any]|None=None)->ArchitectureCheckpoint:
        ...
        plan=self.architect.build_plan(ArchitectureInput(objective=objective,inputs=tuple({"artifact_id":a.artifact_id,"name":a.name,"mime_type":a.mime_type,"uri":a.uri} for a in snapshot.artifacts),context=dict(context or {})));self.sessions.set_architecture_plan(run_id,plan);self.sessions.transition(run_id,WorkflowState.WAITING_ARCHITECT_APPROVAL);gate=self.approvals.request_gate(run_id=run_id,kind="ARCHITECTURE",title="Approve architecture plan",prompt="Review the proposed architecture. Approve to authorize worker execution, modify/clarify to re-plan, or reject to stop the run.",context={"plan":to_dict(plan)});return ArchitectureCheckpoint(run_id,plan,gate.gate_id)
    ...
    def _supervise_recovered_run(self,run_id):
        ...
        if plan is None:
            self.sessions.add_message(run_id,role="system",content="Recovered execution reached SUPERVISING but cannot be QA-validated until its ArchitecturePlan is available.",metadata={"phase":"recovery","status":"blocked_missing_architecture_plan"})
            return None
        return self.supervisor.evaluate(SupervisorInput(run_id=run_id,objective=plan.objective,acceptance_criteria=plan.acceptance_criteria,worker_outputs=tuple(snapshot.worker_outputs.values()),execution_results=snapshot.execution_results,artifacts=tuple(to_dict(a) for a in snapshot.artifacts)))
    def _has_recorded_supervisor_pass(self,run_id):
        for message in reversed(self.sessions.snapshot(run_id).messages):
            if message.metadata.get("phase")=="qa":
                qa=message.metadata.get("qa_result");return isinstance(qa,dict) and qa.get("run_id")==run_id and qa.get("status")==QAStatus.PASS.value
        return False
