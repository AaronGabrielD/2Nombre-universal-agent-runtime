...
`RunExecutionReconciliationService.reconcile_backend(run_id)` performs the same enumeration but explicitly permits the configured `ExecutionReconciler` to provide authoritative evidence for entries that still need backend checking.
...
1. Existing local execution evidence is always preferred and prevents another backend lookup for that execution.
