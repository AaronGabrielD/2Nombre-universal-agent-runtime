...
## M47 corrective hardening

The latest M47 correction round also fixed the production-container and optional-adapter gaps discovered during live validation:

- Production container sets `PYTHONPATH=/app`, includes Docker CLI only, and exposes an HTTP healthcheck.
- Docker backend supplies a valid executable search PATH to its bounded subprocess runner.
- CrewAI remains optional; runtime composition falls back to the native Gemini worker adapter when CrewAI is absent.
- Architect generation receives the configured maximum worker count explicitly before model generation.
- CI builds, starts, and health-checks the production container instead of checking only the CLI version.
- The accidental root-level `placeholder` file was removed.

The separate Chainlit security follow-up remains tracked in issue #79.
